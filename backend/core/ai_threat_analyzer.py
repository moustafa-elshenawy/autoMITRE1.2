...
_SEVERITY_REG_MODEL = None  # SOTA Regression Model

# SOTA Noise Trio: Techniques that often overfit (T1105 Ingress, T1027 Obfuscation, T1082 Discovery)
_SOTA_GATING_LIST = {'T1105', 'T1027', 'T1082'}
...
def _load_databases():
    ...
    try:
        ...
        # Try loading the SOTA custom ML severity model (Classification Fallback)
        model_path = os.path.join(base, '..', 'models', 'severity_model.pkl')
        if os.path.exists(model_path):
            with open(model_path, 'rb') as f:
                _SEVERITY_MODEL = pickle.load(f)
                import logging
                logging.getLogger(__name__).info("Loaded ML Severity Classification Fallback")
        
        # Try loading the ULTRA SOTA ML severity regression model
        reg_model_path = os.path.join(base, '..', 'models', 'severity_regression_model.pkl')
        if os.path.exists(reg_model_path):
            with open(reg_model_path, 'rb') as f:
                _SEVERITY_REG_MODEL = pickle.load(f)
                import logging
                logging.getLogger(__name__).info("Loaded SOTA ML Severity Regression Model")
    except Exception as e:
        import logging
        logging.getLogger(__name__).error(f"Failed to load threat databases/models: {e}")
...
def determine_severity(text: str, technique_ids: List[str]) -> Tuple[SeverityLevel, float, float]:
    """Determine severity, likelihood, and impact scores using SOTA ML model if available."""
    text_lower = text.lower()
    
    # 1. ULTRA SOTA: Regression Model (CVSS-based)
    if _SEVERITY_REG_MODEL is not None:
        try:
            # Regressor outputs a continuous CVSS-equivalent score 0-10
            cvss_score = float(_SEVERITY_REG_MODEL.predict([text])[0])
            
            # Map score to standard SeverityLevel enums
            if cvss_score >= 9.0: mapped_sev = SeverityLevel.CRITICAL
            elif cvss_score >= 7.0: mapped_sev = SeverityLevel.HIGH
            elif cvss_score >= 4.0: mapped_sev = SeverityLevel.MEDIUM
            else: mapped_sev = SeverityLevel.LOW
            
            # Impact is naturally the CVSS score scaled to 0-5
            impact = min(5.0, cvss_score / 2.0)
            # Likelihood is probability-based, boosted by technical density
            likelihood = min(5.0, (cvss_score / 2.5) + min(1.0, len(technique_ids) * 0.2))
            
            return mapped_sev, likelihood, impact
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning(f"SOTA Regression Model failed, falling back: {e}")

    # ML Calibration Path (Old Classification Fallback)
    if _SEVERITY_MODEL is not None:
        try:
            # Predict
            pred = _SEVERITY_MODEL.predict([text])[0]
            # Probabilities for pseudo likelihood/impact
            ...
            mapped_sev = sev_map.get(pred, SeverityLevel.MEDIUM)
            
            # Fabricate bounded impact/likelihood scales based on probability
            base_impact = 1.0
            if mapped_sev == SeverityLevel.CRITICAL: base_impact = 4.5
            elif mapped_sev == SeverityLevel.HIGH: base_impact = 3.5
            elif mapped_sev == SeverityLevel.MEDIUM: base_impact = 2.5
            
            impact = min(5.0, base_impact + (max_prob * 0.5))
            likelihood = min(5.0, max_prob * 3.5 + min(2.0, len(technique_ids) * 0.5))
            
            return mapped_sev, likelihood, impact
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning(f"ML Severity Model failed, falling back: {e}")
    ...
    if 'sql injection' in text_lower:
        if any(kw in text_lower for kw in ['union select', 'drop table', 'truncate', 'sleep(', 'benchmark(']):
            potential_scores.append((SeverityLevel.HIGH, 3.8, 4.5))
        else:
            potential_scores.append((SeverityLevel.MEDIUM, 2.8, 3.2))
