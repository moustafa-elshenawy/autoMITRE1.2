...
export default function TechStackWheel() {
  return (
    <div className="w-full h-full flex flex-col items-center justify-center p-4 gap-4">
      <p className="text-xs font-mono text-slate-400 uppercase tracking-widest">Technology Stack</p>
        ...
        {techGroups.map((group, gi) => (
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              <motion.div
                animate={{ boxShadow: [`0 0 0 ${group.color}`, `0 0 12px ${group.color}80`, `0 0 0 ${group.color}`] }}
                transition={{ duration: 2, repeat: Infinity, delay: gi * 0.4 }}
                className="w-3 h-3 rounded-full shrink-0"
                style={{ backgroundColor: group.color }}
              />
              <span className="text-sm font-bold font-mono" style={{ color: group.color }}>{group.layer}</span>
            </div>
