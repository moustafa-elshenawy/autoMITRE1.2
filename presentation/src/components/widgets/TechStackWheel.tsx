...
export default function TechStackWheel() {
  return (
    <div className="w-full h-full flex flex-col items-center justify-center p-3 gap-3">
      <p className="text-[11px] font-mono text-slate-400 uppercase tracking-widest">Technology Stack</p>
        ...
        {techGroups.map((group, gi) => (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              <motion.div
                animate={{ boxShadow: [`0 0 0 ${group.color}`, `0 0 10px ${group.color}80`, `0 0 0 ${group.color}`] }}
                transition={{ duration: 2, repeat: Infinity, delay: gi * 0.4 }}
                className="w-2.5 h-2.5 rounded-full shrink-0"
                style={{ backgroundColor: group.color }}
              />
              <span className="text-[12px] font-bold font-mono" style={{ color: group.color }}>{group.layer}</span>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {group.techs.map((tech, ti) => (
                <motion.span
                  ...
                  style={{ color: group.color, backgroundColor: `${group.color}14`, border: `1px solid ${group.color}30` }}
                >
                  {tech}
                </motion.span>
              ))}
