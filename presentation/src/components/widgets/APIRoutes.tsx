      ...
      <div className="w-full flex flex-col gap-2.5 max-w-4xl">
        {routes.map((route, i) => (
          <motion.div
            ...
            className="flex items-center gap-4 bg-slate-900/50 border border-slate-800/50 rounded-lg px-4 py-3"
          >
            <span
              className="text-xs font-bold font-mono px-2.5 py-1 rounded shrink-0 min-w-[50px] text-center"
              style={{ color: methodColors[route.method], backgroundColor: `${methodColors[route.method]}20` }}
            >
              {route.method}
            </span>
            <span className="text-sm font-mono text-slate-200 flex-1">{route.path}</span>
            <span className="text-xs font-mono text-slate-500 hidden sm:block shrink-0">→ {route.handler}</span>
            <motion.span
              animate={{ color: route.color, opacity: [1, 0.5, 1] }}
              transition={{ duration: 2, repeat: Infinity, delay: i * 0.3 }}
              className="text-xs font-mono font-bold shrink-0 min-w-[56px] text-right"
            >
              {route.resp}
            </motion.span>
          </motion.div>
