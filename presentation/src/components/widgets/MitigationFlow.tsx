...
export default function MitigationFlow() {
  return (
    <div className="w-full h-full flex flex-col items-center justify-center p-4 gap-6">
      ...
      <div className="flex w-full items-start gap-1.5 relative">
        {steps.map((step, i) => (
          <div key={step.n} className="flex items-center flex-1">
            ...
            >
              <motion.div
                animate={{ boxShadow: [`0 0 0px ${step.color}`, `0 0 20px ${step.color}70`, `0 0 0px ${step.color}`] }}
                transition={{ duration: 2, repeat: Infinity, delay: i * 0.5 }}
                className="w-20 h-20 rounded-2xl border-2 flex flex-col items-center justify-center gap-1.5"
                style={{ borderColor: step.color, backgroundColor: `${step.color}12` }}
              >
                <span className="text-3xl">{step.icon}</span>
                <span className="text-[10px] font-mono font-bold" style={{ color: step.color }}>{step.n}</span>
              </motion.div>
              <div className="text-center px-1">
                <p className="text-sm font-bold font-mono" style={{ color: step.color }}>{step.label}</p>
            ...
            {i < steps.length - 1 && (
              <motion.div
                ...
                className="text-slate-500 text-xl shrink-0 -mt-12"
              >
                →
              </motion.div>
            )}
      ...
      <motion.div
        ...
        className="w-full bg-emerald-950/30 border border-emerald-800/40 rounded-xl p-4 mt-2"
      >
        <p className="text-xs font-mono text-emerald-400 font-bold mb-2">Example Mitigation Output:</p>
